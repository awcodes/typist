            ...
            const controls = `
                <div x-data="{disabled: true}" x-on:click="disabled = false" x-on:click.away="disabled = true">
                    <div class="typist-block-controls" x-show="!disabled" x-cloak>
                        <button type="button" x-bind:disabled="disabled" x-on:click="$wire.mountFormComponentAction('${statePath}', '${node.attrs.identifier}', ${convertValues(node.attrs.values)})">
                        ...
                        <button type="button" x-bind:disabled="disabled" x-on:click="window.editors['${statePath}'].chain().focus().removeBlock().run()">
